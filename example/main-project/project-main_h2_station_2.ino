...
void loop()
{
  ...
  if (strcmp(present_screen_mode, HOME_SCREEN) == 0)
  {
    ...
    if (strcmp(last_screen_mode, HOME_SCREEN) != 0)
    {
      ...
      drawArrayJpeg(station_2, sizeof(station_2), 26.5, 186); // Draw a jpeg image stored in memory
  ...
  if (strcmp(present_screen_mode, WHILE_CHARGING_SCREEN) == 0)
  {
    ...
    if (strcmp(last_screen_mode, HOME_SCREEN) == 0)
    {
      ...
      tft.setTextColor(TFT_DARKGREY, TFT_BLACK);
      tft.println("kg");
      // tft.drawString("kg", tfew + 5, 250, 6);
